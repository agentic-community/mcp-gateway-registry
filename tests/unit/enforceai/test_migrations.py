...
class TestEnforceAIMigrations:
    ...
    def test_upgrade_to_latest_is_idempotent(
        ...
        try:
            ...
            assert [row[0] for row in rows] == [
                ...
                "0007_upstream_oauth_states_ui_return_url",
                "0008_upstream_oauth_providers",
            ]
            ...
            assert _table_exists(connection, table_name="upstream_oauth_providers")
        finally:
            connection.close()
    ...
    def test_downgrade_then_upgrade_restores_schema(
        ...
        try:
            ...
            assert rolled_back == "0008_upstream_oauth_providers"
            ...
            assert _table_exists(connection, table_name="egress_allowlist_entries")
            assert _table_exists(connection, table_name="upstream_oauth_states")
            assert not _table_exists(connection, table_name="upstream_oauth_providers")
            assert _table_exists(connection, table_name="schema_migrations")

            rolled_back = downgrade_one(connection)
            assert rolled_back == "0007_upstream_oauth_states_ui_return_url"
