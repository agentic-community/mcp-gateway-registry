...
- **Agentic Resource Discovery (ARD): Catalog Publisher + Registry** - The registry is now interoperable with the [Agentic Resource Discovery (ARD)](https://github.com/ards-project/ard-spec) spec (v1.0) as both a **Publisher** and a **Registry**, so any ARD-aware client, agent, or peer registry can discover and query its assets through standard interfaces. **Publisher (Phase 1):** a public, no-auth `/.well-known/ai-catalog.json` renders every public + enabled MCP server, A2A agent, and skill as an ARD `catalogEntry` with a domain-anchored URN (`urn:air:<publisher>:<namespace>:<name>`), the correct IANA media type per asset (`application/mcp-server-card+json`, `application/a2a-agent-card+json`, `application/ai-skill`), derived `representativeQueries`/`capabilities`, and an `https` `trustManifest`; entry `url`s point at anonymous per-record endpoints under `/api/public/*` that expose only public records. The rendered manifest passes the ARD `conformance-test manifest` CLI with zero critical errors. **Registry (Phase 2):** `POST /api/ard/search` and `GET /api/ard/agents` expose the existing hybrid semantic-search engine through ARD's search/browse contract — results are ARD `catalogEntry`s plus a `score` (integer 0–100) and `source`, with `query.filter` → entity-type/tag filters and an opaque `pageToken` cursor; browse covers all asset types, deterministically ordered. Both are JWT-required and **access-scoped** (a non-admin caller sees a strict subset, never assets outside its grants), use the ARD `{errorCode, message}` error envelope (including the unauthenticated `401`, enforced at the nginx layer), accept the `federation` parameter (own-index results in Phase 2; cross-registry querying is Phase 3), and emit dedicated `mcpgw_ard_*` metrics. The published catalog also carries a self `application/ai-registry+json` entry linking the two halves. **Federation (Phase 3):** the registry now ingests external `ai-catalog.json` documents (configured per-source like the Anthropic/ASOR/AWS upstreams, managed in **Settings → Federation → External Registries**), validates and indexes their entries into the unified local index, and honors the `federation` parameter on search — `none` (local only), `auto` (merged, source-tagged results), and `referrals` (local results plus `application/ai-registry+json` pointers to peers). Ingestion reuses the peer-sync machinery (origin-tagged, read-only, generation/orphan-tracked), crawls nested catalogs with depth/loop guards, and is protected by an **SSRF guard** (https-only, post-DNS private/metadata-IP blocking, same-domain recursion). **Domain-anchored trust** cross-checks each entry's URN publisher FQDN against the catalog host's `trustManifest.identity` (`reject`/`flag`/`off` policy); ingested items are never re-published in this registry's own catalog. Format adapters over existing data (no new storage), toggled via `ARD_CATALOG_ENABLED` / `ARD_REGISTRY_ENABLED` / `ARD_PUBLISHER_DOMAIN` and wired across Docker, Terraform/ECS, and Helm; ingestion sources live in the DB federation config. CLI: `ard-search` (with `--federation`), `ard-agents`, `ard-ingestion-sync`, `ard-ingestion-status`. ([#1294](https://github.com/agentic-community/mcp-gateway-registry/issues/1294), [#1299](https://github.com/agentic-community/mcp-gateway-registry/pull/1299), [#1295](https://github.com/agentic-community/mcp-gateway-registry/issues/1295), [#1318](https://github.com/agentic-community/mcp-gateway-registry/pull/1318), [#1296](https://github.com/agentic-community/mcp-gateway-registry/issues/1296)) [Catalog Publisher Guide](docs/ard-catalog-publisher.md) | [Registry Adapter Guide](docs/ard-registry.md) | [Federation Guide](docs/ard-federation.md)
