...
- **ARD Registry Adapter (`POST /api/ard/search` + `GET /api/ard/agents`)** - The registry is now a conformant **ARD Registry** (search + browse), exposing its catalog through ARD's standard HTTP interface so any ARD-aware client, agent, or peer registry can query it the same way it queries any other registry. A thin conformance adapter wraps the existing hybrid semantic-search engine: `POST /api/ard/search` returns ARD `catalogEntry` results plus a `score` (integer 0–100, rescaled from the internal 0–1) and `source`, with `query.filter` mapping to entity-type/tag filters and an opaque `pageToken` cursor; `GET /api/ard/agents` browses all asset types (servers + agents + skills), deterministically ordered and paginated. Both are JWT-required and **access-scoped** — results are limited to what the authenticated caller can see (validated end-to-end: a non-admin user sees a strict subset, never another tenant's assets). All `/api/ard/*` errors use the ARD `{errorCode, message}` envelope (the unauthenticated `401` too, enforced at the nginx layer). The `federation` parameter is accepted and returns own-index results in Phase 2 (cross-registry querying is Phase 3); the published `ai-catalog.json` gains a self `application/ai-registry+json` entry linking the Publisher and Registry halves. Every operation emits dedicated `mcpgw_ard_*` metrics. Toggle with `ARD_REGISTRY_ENABLED`; wired across Docker and Helm. CLI: `ard-search`, `ard-agents`. Phase 2 of 3. ([#1295](https://github.com/agentic-community/mcp-gateway-registry/issues/1295), [#1318](https://github.com/agentic-community/mcp-gateway-registry/pull/1318)) [ARD Registry Adapter Guide](docs/ard-registry.md)
