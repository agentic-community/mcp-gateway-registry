...
export function ServerEditModal({
  ...
  useEffect(() => {
    ...
    const providerId = (server.upstream_auth?.provider || '').trim();
    if (providerIds.length > 0 && providerId && !providerIds.includes(providerId)) {
      setUseManualProviderId(true);
      return;
    }
    setUseManualProviderId(false);
  }, [open, providerIds, server.upstream_auth?.provider]);
