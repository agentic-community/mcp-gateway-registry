...
async def _build_origin_map() -> tuple[dict[str, str], list[ArdReferral]]:
    ...
    try:
        ...
        logger.debug(f"Could not load peers for federation type={type(e).__name__}")
    try:
        ...
        logger.debug(
            f"Could not load ai_catalog sources for federation type={type(e).__name__}",
        )
...
async def _override_source_descriptor(
    ...
    try:
        ...
        logger.debug(
            f"Could not load source descriptors for federated results type={type(e).__name__}",
        )
        return
