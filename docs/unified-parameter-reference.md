...
| HTTP pool max connections | `EGRESS_HTTP_POOL_MAX_CONNECTIONS` | `egress_http_pool_max_connections` | `registry.egressAuth.httpPoolMaxConnections` + `auth-server.egressAuth.httpPoolMaxConnections` | Max total connections per egress httpx client pool (shared by url_guard). |
| HTTP pool max keepalive | `EGRESS_HTTP_POOL_MAX_KEEPALIVE` | `egress_http_pool_max_keepalive` | `registry.egressAuth.httpPoolMaxKeepalive` + `auth-server.egressAuth.httpPoolMaxKeepalive` | Max idle keepalive connections kept warm per egress httpx client pool. |
| HTTP pool keepalive expiry (s) | `EGRESS_HTTP_POOL_KEEPALIVE_EXPIRY_SECONDS` | `egress_http_pool_keepalive_expiry_seconds` | `registry.egressAuth.httpPoolKeepaliveExpirySeconds` + `auth-server.egressAuth.httpPoolKeepaliveExpirySeconds` | Seconds an idle keepalive egress connection survives; keep below the shortest upstream/LB idle timeout. |
| HTTP pool connect retries | `EGRESS_HTTP_POOL_CONNECT_RETRIES` | `egress_http_pool_connect_retries` | `registry.egressAuth.httpPoolConnectRetries` + `auth-server.egressAuth.httpPoolConnectRetries` | Connection-establishment retries for egress httpx clients on failure. |
...
| obo cache enabled | `EGRESS_OBO_CACHE_ENABLED` | `egress_obo_cache_enabled` | `registry.egressAuth.oboCacheEnabled` + `auth-server.egressAuth.oboCacheEnabled` | Opt-in (default `false`): cache exchanged OBO tokens in the per-user SecretStore so repeated calls by the same principal to the same audience reuse a still-valid token instead of re-exchanging. Default off preserves the stateless per-request exchange. Read by both the registry and the auth-server. |
| obo cache max TTL (s) | `EGRESS_OBO_CACHE_MAX_TTL_SECONDS` | `egress_obo_cache_max_ttl_seconds` | `registry.egressAuth.oboCacheMaxTtlSeconds` + `auth-server.egressAuth.oboCacheMaxTtlSeconds` | Hard cap (default `300`) on the OBO cache reuse window, applied below the token's real expiry. Bounds revocation / conditional-access re-evaluation latency on cache hits. Read by both the registry and the auth-server. |
| obo cache expiry skew (s) | `EGRESS_OBO_CACHE_EXPIRY_SKEW_SECONDS` | `egress_obo_cache_expiry_skew_seconds` | `registry.egressAuth.oboCacheExpirySkewSeconds` + `auth-server.egressAuth.oboCacheExpirySkewSeconds` | Never reuse a cached OBO token within this many seconds (default `30`) of its expiry. Must be `< EGRESS_OBO_CACHE_MAX_TTL_SECONDS`. Read by both the registry and the auth-server. |
