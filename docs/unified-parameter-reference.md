...
| MCP proxy timeout | `MCP_PROXY_TIMEOUT` | `mcp_proxy_timeout` | `auth-server.app.mcpProxyTimeout` | Timeout (seconds) for the auth-server proxy hop's upstream MCP request; raise for servers with long-running tools. Minimum `1`. Default `30`. Note: values above `60` also require raising `proxy_read_timeout` on the generated `/mcp-proxy/` nginx blocks (they inherit nginx's 60s default), else nginx caps the request first. Terraform var `mcp_proxy_timeout`. |
