...
| PingFederate OAuth client secret **(secret)** | `PINGFEDERATE_CLIENT_SECRET` | `pingfederate_client_secret` | `registry.pingfederate.clientSecret` / `registry.pingfederate.clientSecretExistingSecret` (key `PINGFEDERATE_CLIENT_SECRET`) | **Secret.** Client secret of the `mcp-gateway` OAuth client. On the Compose path `pingfederate/setup/init-pingfederate.sh` *stores* this value as the client's secret; on the Terraform/Helm BYO paths it must match the secret already configured in your PingFederate. **No default — required, fail closed:** `.env.example` ships it empty (it previously shipped `changeme`), the bootstrap rejects unset, whitespace-only, denylisted and sub-12-character values before creating anything, and `build_and_run.sh` rejects the same denylist at preflight when `PINGFEDERATE_ENABLED` is true. Read by registry and auth-server. |
...
| PingFederate admin password **(secret)** | `PF_ADMIN_PASS` | `pf_admin_pass` | `registry.pingfederateAdmin.password` / `registry.pingfederateAdmin.passwordExistingSecret` | Issue #1127. **Secret.** Basic-auth password for the PF admin API. Used by registry to create OAuth clients and Simple PCV users. **No default — required, fail closed.** `.env.example` ships it empty; compose passes `${PF_ADMIN_PASS:-}`; `build_and_run.sh` and `pingfederate/setup/init-pingfederate.sh` both reject the Ping vendor default `2FederateM0re`, as do `registry/utils/pingfederate_manager.py` and the Terraform `pf_admin_pass` validation. With the bundled container this value also bootstraps the console administrator on first start (compose passes it as `PING_IDENTITY_PASSWORD`). Wired through AWS Secrets Manager (Terraform) and `secretKeyRef` (Helm). Read by registry only. |
| PingFederate registry admin login password **(secret)** | `PF_REGISTRY_ADMIN_PASSWORD` | n/a | n/a | **Secret.** Password for the `admin` browser login that `pingfederate/setup/init-pingfederate.sh` seeds into PingFederate's Simple Password Credential Validator and maps to `registry-admins`. **No default — required, fail closed**: the bootstrap rejects unset, whitespace-only, denylisted and sub-12-character values before creating anything, and overwrites (rotates) a pre-existing `admin` row on every run. Bootstrap-script-only; Terraform and Helm do not run that script, so they have no equivalent variable. |
...
| Login base URL | `ENTRA_LOGIN_BASE_URL` | `entra_login_base_url` | Stack chart: `auth-server.entra.loginBaseUrl` (the registry pod reads the same shared `oauth-provider-secret`). Standalone registry chart: `entra.loginBaseUrl`. | Sovereign clouds. Read by BOTH the auth-server (user login, OBO) and the registry (its own `client_credentials` grant for headless backend discovery/health against `obo_exchange` servers), so it must reach both containers. **In stack mode, setting `registry.entra.loginBaseUrl` has no effect** — the umbrella renders only the `auth-server` value, as it does for every other Entra setting. |
...
| Score threshold | `DEDUP_SCORE_THRESHOLD` | `dedup_score_threshold` | `registry.app.dedup.scoreThreshold` | Minimum cosine similarity (0.0..1.0) for a candidate to surface. **Leave unset** and the registry uses the default calibrated for `EMBEDDINGS_MODEL_NAME`: `all-MiniLM-L6-v2` → `0.45`, `text-embedding-ada-002` → `0.85`, anything else → `0.6` plus a startup warning. Cosine scales are model-specific and not comparable — measured on one corpus, MiniLM puts unrelated pairs at 0.00-0.25 and duplicates at 0.50-0.83, while ada-002 puts unrelated pairs at 0.73-0.78 (gibberish scores 0.736) and duplicates at 0.83-0.92. A value here overrides the per-model default; the registry logs which source it used. |
...
| Egress HTTP pool tuning | `EGRESS_HTTP_POOL_MAX_CONNECTIONS`, `EGRESS_HTTP_POOL_MAX_KEEPALIVE`, `EGRESS_HTTP_POOL_KEEPALIVE_EXPIRY_SECONDS`, `EGRESS_HTTP_POOL_CONNECT_RETRIES` | `egress_http_pool_max_connections`, `egress_http_pool_max_keepalive`, `egress_http_pool_keepalive_expiry_seconds`, `egress_http_pool_connect_retries` | `mcpgw.app.httpPoolMaxConnections`, `mcpgw.app.httpPoolMaxKeepalive`, `mcpgw.app.httpPoolKeepaliveExpirySeconds`, `mcpgw.app.httpPoolConnectRetries` | mcpgw's outbound calls (the 7 Registry API calls + the Keycloak M2M `client_credentials` token POST) share one process-lifetime pooled `httpx.AsyncClient`. Same four variables, defaults and bounds as the registry/auth-server — full descriptions in **Group 31** below, operations guide in [`docs/egress-http-client-pooling.md`](./egress-http-client-pooling.md). The Terraform variables are shared across the three services; mcpgw parses the env vars itself in `servers/mcpgw/http_pool.py` (its image contains no `registry` package), so set them identically on registry, auth-server and mcpgw. |
...
| Audit TTL (days) | `AUDIT_LOG_MONGODB_TTL_DAYS` | `audit_log_ttl_days` | `mongodb-configure.mongodb.auditTtlDays` | TTL index on `audit_events`, default 7. Set it in Helm values rather than on the collection: the init Job re-runs on every `helm upgrade`, so an out-of-band value is not durable. |
| Allow shortening audit TTL | `AUDIT_LOG_MONGODB_TTL_ALLOW_SHRINK` | — | `mongodb-configure.mongodb.auditTtlAllowShrink` | Default `false`. Reducing the TTL makes the database delete every audit record older than the new window, so the init script refuses and keeps the longer retention unless this is true. |
| MongoDB wait timeout (s) | `MONGODB_WAIT_TIMEOUT_SECONDS` | — | `mongodb-configure.job.mongodbWaitTimeoutSeconds` | Default 300. How long the init script waits for MongoDB to accept connections and for its replica set to become ready before failing. |
| Replica-set initiate grace (s) | `MONGODB_REPLICA_SET_INITIATE_GRACE_SECONDS` | — | — | Default 30. How long the init script waits for a managed control plane (the MongoDB Kubernetes operator) to configure the replica set before initializing it itself. |
| Init Job deadline (s) | — | — | `mongodb-configure.job.activeDeadlineSeconds` | Default 1800. Whole-attempt ceiling for the init Job, covering the MongoDB wait plus index builds. Raise it for a large audit collection, and keep it inside `helm upgrade --timeout`. |
| Init Job retention (s) | — | — | `mongodb-configure.job.ttlSecondsAfterFinished` | Default 3600. Helm does not delete hook resources on `helm uninstall`, so the finished Job cleans itself up. |
...
| HTTP pool max connections | `EGRESS_HTTP_POOL_MAX_CONNECTIONS` | `egress_http_pool_max_connections` | `registry.egressAuth.httpPoolMaxConnections` + `auth-server.egressAuth.httpPoolMaxConnections` + `mcpgw.app.httpPoolMaxConnections` | Max total connections per egress httpx client pool (shared by url_guard; mcpgw parses the same var itself in `servers/mcpgw/http_pool.py`). |
| HTTP pool max keepalive | `EGRESS_HTTP_POOL_MAX_KEEPALIVE` | `egress_http_pool_max_keepalive` | `registry.egressAuth.httpPoolMaxKeepalive` + `auth-server.egressAuth.httpPoolMaxKeepalive` + `mcpgw.app.httpPoolMaxKeepalive` | Max idle keepalive connections kept warm per egress httpx client pool. Clamped to max connections at startup. |
| HTTP pool keepalive expiry (s) | `EGRESS_HTTP_POOL_KEEPALIVE_EXPIRY_SECONDS` | `egress_http_pool_keepalive_expiry_seconds` | `registry.egressAuth.httpPoolKeepaliveExpirySeconds` + `auth-server.egressAuth.httpPoolKeepaliveExpirySeconds` + `mcpgw.app.httpPoolKeepaliveExpirySeconds` | Seconds an idle keepalive egress connection survives; keep below the shortest upstream/LB idle timeout. `0` disables keep-alive reuse — the runtime rollback lever for all three processes. |
| HTTP pool connect retries | `EGRESS_HTTP_POOL_CONNECT_RETRIES` | `egress_http_pool_connect_retries` | `registry.egressAuth.httpPoolConnectRetries` + `auth-server.egressAuth.httpPoolConnectRetries` + `mcpgw.app.httpPoolConnectRetries` | Connection-establishment retries for egress httpx clients on failure. |
