...
| Trusted egress OAuth IdP hosts | `EGRESS_OAUTH_TRUSTED_IDP_HOSTS` | n/a (set via `extraEnv`) | n/a (set via `extraEnv`) | Comma-separated exact hostnames of operator-controlled OAuth/OIDC identity providers whose **token endpoints** may resolve to private addresses (e.g. `keycloak.internal.example.com`). Needed when the IdP backing per-user egress consent is self-hosted on an internal network, which the credentialed-OAuth profile blocks by default — consent otherwise fails with `token endpoint blocked by security policy`. The two SSRF settings above deliberately do **not** apply to token POSTs, since those carry client secrets, refresh tokens or user assertions and must not inherit a proxy-target bypass. Hosts only: no CIDRs and no wildcards. HTTPS is still required, answers are still resolved, classified and pinned, and cloud/workload credential, metadata and link-local addresses are never permitted. Empty = only public token endpoints allowed. |
