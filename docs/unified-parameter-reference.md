...
|---|---------|----------------|-----------------------------|--------------------------------------------|
| 1 | **Docker / docker-compose** (local dev + single-host prod) | `SCREAMING_SNAKE_CASE` env vars | `.env` (copy from [`.env.example`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/.env.example)). Also wired into: [`docker-compose.yml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/docker-compose.yml), [`docker-compose.podman.yml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/docker-compose.podman.yml), [`docker-compose.prebuilt.yml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/docker-compose.prebuilt.yml), [`docker-compose.dhi.yml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/docker-compose.dhi.yml) | `docker exec <container> env \| grep <VAR>`, and the UI at **Settings → System Config → Configuration** (also [`GET /api/config/full`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/registry/api/config_routes.py)) |
| 2 | **Terraform / AWS ECS** (managed AWS deployment) | `snake_case` Terraform variables | `terraform/aws-ecs/terraform.tfvars` (copy from [`terraform.tfvars.example`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/terraform.tfvars.example)). Also wired into: [`variables.tf`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/variables.tf), [`main.tf`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/main.tf), [`modules/mcp-gateway/variables.tf`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/modules/mcp-gateway/variables.tf), [`modules/mcp-gateway/ecs-services.tf`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/modules/mcp-gateway/ecs-services.tf) | `terraform plan`, ECS console task-definition env block, CloudWatch logs, or hit `/api/config/full` on the deployed ALB |
| 3 | **Helm / Kubernetes (EKS)** | `camelCase` Helm values | [`charts/mcp-gateway-registry-stack/values.yaml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/charts/mcp-gateway-registry-stack/values.yaml) for the full stack, or per-subchart: [`charts/registry/values.yaml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/charts/registry/values.yaml), [`charts/auth-server/values.yaml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/charts/auth-server/values.yaml), [`charts/mcpgw/values.yaml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/charts/mcpgw/values.yaml), [`charts/mongodb-configure/values.yaml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/charts/mongodb-configure/values.yaml), [`charts/keycloak-configure/values.yaml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/charts/keycloak-configure/values.yaml). Container env wired in each chart's `templates/deployment.yaml` and `templates/secret.yaml`. | `helm template charts/mcp-gateway-registry-stack \| grep <VAR>`, `kubectl describe pod <pod> \| grep <VAR>`, or hit `/api/config/full` on the ingress |
...
| `GET /api/config/full` | Admin only, **cookie session only** — Bearer JWT does not currently work even for admin users (see note below) | Grouped full configuration. Sensitive values masked. Wired in [`registry/api/config_routes.py`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/registry/api/config_routes.py) |
...
| **Docker (`.env`)** | Variable name in [`.env.example`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/.env.example) / `.env`. |
| **Terraform (`.tfvars`)** | Variable name in [`terraform/aws-ecs/terraform.tfvars.example`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/terraform.tfvars.example). Blank = not exposed on this surface (deployment-agnostic or not yet wired). |
...
- [ ] Add to [`.env.example`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/.env.example) with description and default.
- [ ] Wire through `docker-compose.yml` (and `.podman.yml`, `.prebuilt.yml`, `.dhi.yml`) service env blocks.
- [ ] Add Terraform variable in [`terraform/aws-ecs/variables.tf`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/variables.tf), pass through [`main.tf`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/main.tf) and the module, map to ECS task env in [`modules/mcp-gateway/ecs-services.tf`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/modules/mcp-gateway/ecs-services.tf), and document in [`terraform.tfvars.example`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/terraform/aws-ecs/terraform.tfvars.example).
- [ ] Add Helm values default in [`charts/<subchart>/values.yaml`](https://github.com/agentic-community/mcp-gateway-registry/tree/main/charts) AND the stack [`charts/mcp-gateway-registry-stack/values.yaml`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/charts/mcp-gateway-registry-stack/values.yaml); wire into the subchart's `templates/deployment.yaml` and (if sensitive) `templates/secret.yaml`.
- [ ] Register the field in [`registry/api/config_routes.py`](https://github.com/agentic-community/mcp-gateway-registry/blob/main/registry/api/config_routes.py) `CONFIG_GROUPS` so it appears on **Settings → System Config** and in `GET /api/config/full`. Mark sensitive values with `is_sensitive=True`.
