...
| MCP proxy upstream timeout | `MCP_PROXY_UPSTREAM_TIMEOUT_SECONDS` | `mcp_proxy_upstream_timeout_seconds` | `auth-server.app.mcpProxyUpstreamTimeoutSeconds`, `registry.app.mcpProxyUpstreamTimeoutSeconds` | Per-request timeout (seconds) the auth-server proxy hop applies to the upstream MCP server call; on expiry the hop returns HTTP 504. Raise for upstreams whose tool calls legitimately run long (e.g. multi-cluster A2A coordinators). Minimum `1`. Default `30`. Set it on **both** deployments: the registry derives its nginx `proxy_read_timeout`/`proxy_send_timeout` for the MCP location blocks from this value plus a 30s headroom buffer (default → `60s`), so a single knob raises the whole proxy chain and the inner auth-server hop always times out first. |
