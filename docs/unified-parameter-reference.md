...
| Signing key path **(secret)** | `INTERNAL_SIGNING_KEY_PATH` | `internal_signing_key_secret_arn` | `auth-server.signingKey.enabled` + volume mount | Path to ES256 PEM private key. When set, auth-server signs JWTs with ES256 instead of HS256 (SECRET_KEY). Empty = HS256 fallback. |
| Signing key ID | `INTERNAL_SIGNING_KEY_ID` | `internal_signing_key_id` | `auth-server.signingKey.kid` | Key ID (kid) stamped into ES256-signed JWTs. Increment on rotation. Default `es256-1`. |
| Reject HS256 tokens | `REJECT_HS256_TOKENS` | `reject_hs256_tokens` | `auth-server.signingKey.rejectHs256` | Hard-reject legacy HS256 tokens. Enable only after all tokens rotated to ES256 (>24h). Default `false`. |
| CSRF signing key **(secret)** | `CSRF_SIGNING_KEY` | — | — | Registry-only CSRF token signing. Falls back to SECRET_KEY when empty. Never shared with auth-server. |
| Credential encryption key **(secret)** | `CREDENTIAL_ENCRYPTION_KEY` | — | — | Registry-only backend server credential encryption (Fernet). Falls back to SECRET_KEY when empty. Changing invalidates existing encrypted credentials. |
| Session token encryption key **(secret)** | `SESSION_TOKEN_ENC_KEY` | — | — | id_token encryption at rest (AES-GCM). Shared: auth-server + registry. Falls back to SECRET_KEY when empty. Changing invalidates stored encrypted id_tokens (users must re-login). |
