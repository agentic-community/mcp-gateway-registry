...
| Signing key path **(secret)** | `INTERNAL_SIGNING_KEY_PATH` | `internal_signing_key_secret_arn` | `auth-server.signingKey.enabled` + volume mount | Path to ES256 PEM private key (default mount `/etc/mcp-gateway/signing-key/key.pem`). When set, auth-server signs JWTs with ES256 instead of HS256 (SECRET_KEY); empty = HS256 fallback. The `kid` is **auto-derived from the key's RFC 7638 JWK thumbprint** — no operator-set id. It is stable per key and identical across replicas, so rotation and multi-replica overlap never collide. See [asymmetric signing](design/asymmetric-signing.md). |
| Reject HS256 tokens | `REJECT_HS256_TOKENS` | `reject_hs256_tokens` | `auth-server.signingKey.rejectHs256`, `registry.app.rejectHs256` | Hard-reject legacy HS256 (no-`kid`) internal tokens. Read by **both** the auth-server (minter) and the registry (verifier), so it **must be set on both services** — setting it only on auth-server leaves the registry accepting forged HS256 tokens. Enable only after all tokens have rotated to ES256 (allow one max-TTL window). Accepts `1`/`true`/`yes`/`on`. Default `false`. |
| CSRF signing key **(secret)** | `CSRF_SIGNING_KEY` | `csrf_signing_key_secret_arn` | `registry.app.csrfSigningKey` | Registry-only CSRF token signing. Falls back to SECRET_KEY when empty. Never shared with auth-server. |
| Credential encryption key **(secret)** | `CREDENTIAL_ENCRYPTION_KEY` | `credential_encryption_key_secret_arn` | `registry.app.credentialEncryptionKey` | Registry-only backend server credential encryption (Fernet). Falls back to SECRET_KEY when empty. Changing invalidates existing encrypted credentials. |
| Session token encryption key **(secret)** | `SESSION_TOKEN_ENC_KEY` | `session_token_enc_key_secret_arn` | `registry.app.sessionTokenEncKey`, `auth-server.app.sessionTokenEncKey` | id_token encryption at rest (AES-GCM). Shared: auth-server + registry (set on both charts). Falls back to SECRET_KEY when empty. Changing invalidates stored encrypted id_tokens (users must re-login). |
| Internal JWKS URL | `INTERNAL_JWKS_URL` | `internal_jwks_url` | `registry.app.internalJwksUrl` | Registry-side URL for auth-server's internal JWKS; the registry fetches the ES256 public key(s) here to verify hop tokens. Defaults to the bare service name (`http://auth-server:8888/.well-known/internal-jwks.json`) which resolves on Docker Compose and ECS (Service Connect); the Helm chart defaults it to the in-namespace FQDN (`auth-server.<ns>.svc.cluster.local`) because the bare name does not reliably resolve from the registry pod on EKS. Unused while auth-server signs HS256. |
| Internal JWKS cache TTL | `INTERNAL_JWKS_CACHE_TTL_SECONDS` | `internal_jwks_cache_ttl_seconds` | `registry.app.internalJwksCacheTtlSeconds` | How long the registry caches the internal JWKS before re-fetching; caps `kid`-rotation propagation. On fetch failure, last-known-good keys are served up to a bounded staleness. Default `300`. |
