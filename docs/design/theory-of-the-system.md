...
### 2.2 The registry is a control plane, never the data path, for agent-to-agent traffic
...
### 2.3 Gateway vs registry-only is an explicit, configuration-level distinction
...
### 2.4 Configuration parity across the three deployment surfaces is an invariant, not a courtesy
...
### 2.5 Admission fails closed; notification fails open
...
### 2.6 Auth is agnostic across a supported set of IdPs — but that set is a closed, hand-written factory
...
### 2.7 MCP spec compliance is a first-class, tracked commitment
...
- **It is not part of A2A data traffic** (§2.2) — discovery and auth only.
...
- **New IdP support is a code change, not zero-config discovery** (§2.6).
...
2. **New outbound or agent-traffic path?** Confirm you are not turning the registry into a data
   plane it is designed not to be (§2.2).
3. **New mode-dependent behavior?** Gate it on the correct axis — `DEPLOYMENT_MODE` for
   gateway-vs-registry, `REGISTRY_MODE` for entity-type enablement (§2.3).
4. **New configuration parameter?** Wire it through all three surfaces and update
   [`docs/unified-parameter-reference.md`](../unified-parameter-reference.md) (§2.4).
5. **New external integration?** Decide explicitly whether it is admission (fail closed) or
   notification (fail open) — and implement the matching failure mode (§2.5).
6. **Auth change?** Keep it provider-agnostic across the supported six; do not leak one provider's
   assumptions into shared code (§2.6).
7. **MCP-facing endpoint?** Preserve the PRM / `WWW-Authenticate` discovery contract (§2.7).
