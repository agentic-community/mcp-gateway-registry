...
- Status of the modes: **`none` (no egress auth), `vault-oauth` (3LO), `token-exchange` (OBO), and `vault-pat` (PAT) are implemented today** (OBO via Microsoft Entra `jwt-bearer` and Keycloak RFC 8693 token exchange). The **static operator upstream auth headers** capability that the old `custom-header` placeholder anticipated is now **implemented** for the generic proxy — encrypted at rest and vended registry -> auth-server (see the modes table and the [gateway generic proxy design](gateway-generic-proxy.md)).
...
- **TLS is required.** `CREDENTIALED_OAUTH_PROFILE` sets `require_https=True`; trusted-host naming relaxes the public-address requirement, not TLS. The shipped in-cluster default (`KEYCLOAK_URL=http://keycloak:8080`) therefore cannot serve OBO until Keycloak is fronted with `https://`. An internal CA works, but there is no application-level CA-bundle setting for this hop today (`KEYCLOAK_CA_BUNDLE` is consumed by nginx only). The auth-server verifies TLS against the certifi bundle: its httpx clients, including this token POST, read `SSL_CERT_FILE`, and the Keycloak provider's `requests` calls to `KEYCLOAK_URL` (JWKS, token, userinfo) read `REQUESTS_CA_BUNDLE`. Each variable **replaces** the certifi bundle rather than extending it, so point both at one combined file holding the public roots plus the internal CA; a file with only the internal CA breaks every public TLS call the process makes. In-cluster **non-TLS** OBO is intentionally unsupported; an explicit, operator-gated override is a possible future extension, out of scope for the current implementation.
- **The subject token's issuer must match the host the exchange is sent to.** When Keycloak derives its hostname from each request (development mode, or `hostname-strict=false`), it rejects the subject token if its `iss` does not match the realm URL of the host that receives the exchange (observed with legacy exchange: a token issued via `localhost` is refused through the `keycloak` host). Pin `KC_HOSTNAME` to a full URL (scheme, host and port) so every token carries one issuer, or send the exchange to the same host that issued the user's token.
- **Keep gateway audiences out of the exchanged token.** Keycloak has two token-exchange implementations. Legacy exchange (V1, the `token-exchange` feature) is the only one up to 26.1 and remains a preview on 26.2+, where it still serves a request whenever it is enabled and the gateway client's *Standard token exchange* switch is off (the default); on 26.2+ it also requires fine-grained admin permissions version 1. It mints the token with the audience client's default client scopes and only adds the requested audience, so the token inherits every audience those scopes add. The realm set up by `keycloak/setup/init-keycloak.sh` attaches an `mcp-gateway` audience mapper to the `basic` scope, which every client carries, so a legacy-exchanged token would also be a valid credential for this gateway, and the upstream could replay it as the user. The gateway therefore refuses, with a configuration error, any exchanged token whose `aud` contains an audience it accepts (`mcp-gateway`, the web or the M2M client id). With the shipped realm this applies to the compose and Terraform surfaces (Keycloak 25.0) and, by default, to the CDK surface (Keycloak 26.2.4 with `token-exchange` enabled). OBO works there once one of these is done:
  - **Use standard token exchange** (Keycloak 26.2+; the Helm surface's 26.3.3 has it on by default). Three things must hold. Turn on *Standard token exchange* for the gateway client. The gateway client must be in the subject token's `aud` unless the token was issued to it — the shipped realm adds the `mcp-gateway` audience, not the gateway client's id — otherwise Keycloak answers `access_denied` ("Client is not within the token audience"). And the target audience must be available to the gateway client: the `audience` parameter only narrows `aud` and cannot add to it, otherwise Keycloak answers `invalid_request` ("Requested audience not available"). One way that keeps the target out of the users' ingress tokens is an *optional* client scope on the gateway client, named exactly like the target client and carrying an audience mapper for it, requested per server through `egress_oauth.scopes` (registration accepts a scope named like the target); an audience mapper on one of the gateway client's *default* scopes would instead put the target audience into every token issued to that client, including the ingress tokens. Standard exchange then filters `aud` down to the requested audience. This path is derived from Keycloak's documentation and source; the live check covered legacy exchange only.
  - **Or, with legacy exchange,** replace `basic` on the target client with a copy that keeps its `sub`, `auth_time` and `groups` mappers and drops only the `mcp-gateway` audience mapper. Removing `basic` outright also removes `sub`, and the gateway refuses an exchanged token without it.
