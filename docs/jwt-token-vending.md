...
## Token Rotation (No Background Refresher)

- Vended gateway tokens are intentionally short-lived; rotate by issuing a new token before expiry.
- Upstream authentication is gateway-managed; agents should not store or refresh upstream provider tokens locally.
